    ...
    public class Entity_EdgeGroup : Entity
    {
        ...
        private void Update()
        {
            if (!isRim && (primaryRoom != null || secondaryRoom != null))
            {
                ...
                if (parentEdge.corner != null)
                {
                    parentEdge.corner.SetWallHide(value);
                }
        ...
        {
            parentEdge = parent;
            parentEdge.entity = this;

            primaryCell = p;
            primaryRoom = primaryCell.parentRoom;

            secondaryCell = s;
            secondaryRoom = secondaryCell.parentRoom;

            m_WallPos = (primaryCell.worldPosition + secondaryCell.worldPosition) / 2f;

            if (primaryCell.Data.CellType == FlowTilemapCellType.Custom || secondaryCell.Data.CellType == FlowTilemapCellType.Custom)
            {
                isRim = true;
            }
        }
        ...
        public void OnDrawGizmos()
        {
            if (drawGizmos)
            {
                ...
                var parentPosition = DungeonController.Instance.TileCoordToWorldPosition(parentEdge.Data.EdgeCoord);
