Shader "CrossSectionHDRP/Renderers/CapWrite"
{
	Properties
	{
		_Color("Color", Color) = (1,1,1,1)
		//_ColorMap("ColorMap", 2D) = "white" {}

		// Transparency
		//_AlphaCutoff("Alpha Cutoff", Range(0.0, 1.0)) = 0.5
		[Enum(UnityEngine.Rendering.CullMode)] _Culling ("Cull Mode", Int) = 0
		_StencilMask("Stencil Mask", Range(0, 255)) = 255
		[Enum(None,0,Alpha,1,Red,8,Green,4,Blue,2,RGB,14,RGBA,15)] _ColorMask("Color Mask", Int) = 0
		_StencilWriteMask("_StencilWriteMask", Float) = 192
		_StencilReadMask("_StencilReadMask", Float) = 192
		[Enum(UnityEngine.Rendering.StencilOp)] _StencilOpFront("OpFront", Int) = 1
		[Enum(UnityEngine.Rendering.StencilOp)] _StencilOpBack("OpBack", Int) = 2
		//[Enum(Keep,0,Zero,1,Replace,2,IncrementSaturate,3,DecrementSaturate,4,Invert,5,IncrementWrap,6,DecrementWrap,7)] _StencilOpBack("OpBack", Int) = 2
	}

	HLSLINCLUDE

	#pragma target 4.5
	#pragma only_renderers d3d11 playstation xboxone vulkan metal switch

	// #pragma enable_d3d11_debug_symbols

	//enable GPU instancing support
	#pragma multi_compile_instancing
	#pragma multi_compile __ CLIP_BOX CLIP_CORNER CLIP_PLANE CLIP_SPHERE_OUT
	#pragma multi_compile _ DOTS_INSTANCING_ON

	ENDHLSL

	SubShader
	{
		Tags{ "RenderPipeline" = "HDRenderPipeline" }
		Pass
		{
			Name "FirstPass"
			Tags { "LightMode" = "FirstPass" }

			Blend Off
			//ZWrite Off
			ZTest LEqual
			Cull [_Culling]

			ColorMask[_ColorMask]

			Stencil
			{
				Ref[_StencilMask]
				WriteMask [_StencilWriteMask]
				ReadMask [_StencilReadMask]
				Comp Always
				PassBack [_StencilOpBack]
				PassFront [_StencilOpFront]
				//Pass Replace
			}

			HLSLPROGRAM

			// Toggle the alpha test
			#define _ALPHATEST_ON

			// Toggle transparency
			// #define _SURFACE_TYPE_TRANSPARENT

			// Toggle fog on transparent
			//#define _ENABLE_FOG_ON_TRANSPARENT

			// List all the attributes needed in your shader (will be passed to the vertex shader)
			// you can see the complete list of these attributes in VaryingMesh.hlsl
			//#define ATTRIBUTES_NEED_TEXCOORD0
			//#define ATTRIBUTES_NEED_NORMAL
			//#define ATTRIBUTES_NEED_TANGENT

			// List all the varyings needed in your fragment shader
			//#define VARYINGS_NEED_TEXCOORD0
			//#define VARYINGS_NEED_TANGENT_TO_WORLD

			#define CAP_PREPARE

			#define VARYINGS_NEED_POSITION_WS
			#include "Packages/com.unity.render-pipelines.core/ShaderLibrary/Common.hlsl"
			half _inverse = 0;

            // Declare properties in the UnityPerMaterial cbuffer to make the shader compatible with SRP Batcher.
CBUFFER_START(UnityPerMaterial)
            float4 _Color;
            float _StencilMask;
			float _ColorMask;
            float _StencilWriteMask;
CBUFFER_END
            #include "Packages/com.unity.render-pipelines.high-definition/Runtime/RenderPipeline/RenderPass/CustomPass/CustomPassRenderersV2.hlsl"
			#include "section_clipping_CS.hlsl"
            // If you need to modify the vertex datas, you can uncomment this code
            // Note: all the transformations here are done in object space
            // #define HAVE_MESH_MODIFICATION
            // AttributesMesh ApplyMeshModification(AttributesMesh input, float3 timeParameters)
            // {
            //     input.positionOS += input.normalOS * 0.0001; // inflate a bit the mesh to avoid z-fight
            //     return input;
            // }

            // Put the code to render the objects in your custom pass in this function

            void GetSurfaceAndBuiltinData(FragInputs fragInputs, float3 viewDirection, inout PositionInputs posInput, out SurfaceData surfaceData, out BuiltinData builtinData)
            {	
                // Write back the data to the output structures
				ZERO_BUILTIN_INITIALIZE(builtinData); // No call to InitBuiltinData as we don't have any lighting
				ZERO_INITIALIZE(SurfaceData, surfaceData);
                surfaceData.color = _Color.rgb;
            }

            #include "ShaderPassForwardUnlit.hlsl"

            #pragma vertex Vert
            #pragma fragment Frag

            ENDHLSL
        }
    }
}
