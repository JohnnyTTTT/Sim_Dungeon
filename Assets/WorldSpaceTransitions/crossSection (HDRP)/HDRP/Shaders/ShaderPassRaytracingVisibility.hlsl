...
void AnyHitVisibility(inout RayIntersectionVisibility rayIntersection : SV_RayPayload, AttributeData attributeData : SV_IntersectionAttributes)
{
    ...
    float3 wpos = GetAbsolutePositionWS(posInput.positionWS);//CrossSection
    float3 wpos = posInput.positionWS;
    bool sectionNotVisible;
    SECTION_VCLIP(wpos, sectionNotVisible);//CrossSection
    isVisible = isVisible && !sectionNotVisible;
    #endif
