...
//#if ENABLE_LEGACY_INPUT_MANAGER
        ...
        if (plane)
        {
            if (transform.position.y < plane.transform.position.y) transform.position = new Vector3(transform.position.x, plane.transform.position.y, transform.position.z);
        }
    }
