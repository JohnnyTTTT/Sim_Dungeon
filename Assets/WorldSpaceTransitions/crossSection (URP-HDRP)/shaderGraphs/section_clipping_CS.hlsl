...
void ClipPie_float(float3 posWorld, float alphaClipThreshold, bool isFrontFace, out float Out)
{
	...
	{
		Out = ClipPie(posWorld, _WorldToObjectMatrix, _pieAngle) ? 2.0 : alphaClipThreshold;
	}
}
...
static const int _primCountTruncated = min(_primCount, 64);//let's assume 64 as maximum prism count expected
